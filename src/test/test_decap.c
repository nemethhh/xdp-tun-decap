 ...
 */
static int run_xdp_test(int prog_fd,
                        ...
                        void *data_out, size_t *data_out_len)
{
    LIBBPF_OPTS(bpf_test_run_opts, opts,
        ...
        .data_out = data_out,
        .data_size_out = data_out_len ? *data_out_len : 0,
        .repeat = 1,
    );
...
static void test_gre_whitelisted(struct tun_decap_bpf *skel)
{
    ...
    err = run_xdp_test(prog_fd,
                       pkt_gre_whitelisted, PKT_GRE_WHITELISTED_LEN,
                       &retval,
    ...
    if (retval != XDP_PASS) {
        char buf[64];
        snprintf(buf, sizeof(buf), "Expected XDP_PASS(%d), got %d",
                 XDP_PASS, retval);
        TEST_FAIL(name, buf);
        return;
    }
...
static void test_gre_blocked(struct tun_decap_bpf *skel)
{
    ...
    err = run_xdp_test(prog_fd,
                       pkt_gre_blocked, PKT_GRE_BLOCKED_LEN,
                       &retval,
                       NULL, NULL);
    if (err < 0) {
        TEST_FAIL(name, "bpf_prog_test_run failed");
    ...
    if (retval != XDP_DROP) {
        char buf[64];
        snprintf(buf, sizeof(buf), "Expected XDP_DROP(%d), got %d",
                 XDP_DROP, retval);
        TEST_FAIL(name, buf);
        return;
    }
...
static void test_gre_with_key(struct tun_decap_bpf *skel)
{
    ...
    /* Run test */
    err = run_xdp_test(prog_fd,
                       pkt_gre_with_key, PKT_GRE_WITH_KEY_LEN,
                       &retval,
                       data_out, &data_out_len);
    if (err < 0) {
        TEST_FAIL(name, "bpf_prog_test_run failed");
...
static void test_ipip_whitelisted(struct tun_decap_bpf *skel)
{
    ...
    /* Run test */
    err = run_xdp_test(prog_fd,
                       pkt_ipip_whitelisted, PKT_IPIP_WHITELISTED_LEN,
                       &retval,
                       data_out, &data_out_len);
    if (err < 0) {
        TEST_FAIL(name, "bpf_prog_test_run failed");
...
static void test_ipip_blocked(struct tun_decap_bpf *skel)
{
    ...
    err = run_xdp_test(prog_fd,
                       pkt_ipip_blocked, PKT_IPIP_BLOCKED_LEN,
                       &retval,
                       NULL, NULL);
    if (err < 0) {
        TEST_FAIL(name, "bpf_prog_test_run failed");
...
static void test_tcp_passthrough(struct tun_decap_bpf *skel)
{
    ...
    err = run_xdp_test(prog_fd,
                       pkt_tcp_normal, PKT_TCP_NORMAL_LEN,
                       &retval,
                       data_out, &data_out_len);
    if (err < 0) {
        TEST_FAIL(name, "bpf_prog_test_run failed");
...
static void test_udp_passthrough(struct tun_decap_bpf *skel)
{
    ...
    err = run_xdp_test(prog_fd,
                       pkt_udp_normal, PKT_UDP_NORMAL_LEN,
                       &retval,
                       data_out, &data_out_len);
    if (err < 0) {
        TEST_FAIL(name, "bpf_prog_test_run failed");
...
static void test_gre_truncated(struct tun_decap_bpf *skel)
{
    ...
    err = run_xdp_test(prog_fd,
                       pkt_gre_truncated, PKT_GRE_TRUNCATED_LEN,
                       &retval,
                       NULL, NULL);
    if (err < 0) {
        TEST_FAIL(name, "bpf_prog_test_run failed");
